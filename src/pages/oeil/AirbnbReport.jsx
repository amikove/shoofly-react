...
export default function AirbnbReport() {
  ...
  // Visibilité de l'autosave (audit 360° v4 RG7 : l'échec était totalement avalé par un
  // .catch(() => {}), sans miroir local ni garde de sortie — contrairement à AuditReport.jsx).
  ...
  // Clé propre au rapport Airbnb — distincte de `shoofly_audit_draft_*` (AuditReport.jsx) pour
  // qu'un Œil ayant les deux écrans ouverts ne voie jamais l'un écraser le brouillon de l'autre.
  const DRAFT_KEY = `shoofly_airbnb_draft_${missionId}`
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      reportsAPI.get(missionId)
        ...
        .catch(() => {
          // Plus jamais avalé en silence (RG7) : l'Œil voit « Échec de sauvegarde » et un
          // miroir local est écrit pour pouvoir restaurer la saisie.
          setSaveState('error')
          try {
  ...
  // encore parties côté serveur (audit 360° v4 RG7). Armée UNIQUEMENT quand c'est le cas —
  ...
  const save = async () => {
    ...
    try {
      ...
      toast(t('oeilAirbnbReport.toasts.saveError'), 'error')
    }
        ...
        {!submitted && localDraft && (
          <div className="card mb-4 bg-amber-500/10 border border-amber-500/30">
            <p className="text-xs text-amber-300 mb-2">
              {t('oeilAirbnbReport.localDraft.notice', { time: new Date(localDraft.savedAt).toLocaleString('fr-FR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) })}
            </p>
            <div className="flex gap-2">
              <button
      ...
      {!submitted && (
        <div className="fixed bottom-20 md:bottom-0 start-0 end-0 md:start-[220px] bg-[#181818] border-t border-white/12 p-4 flex flex-col gap-2 z-40">
          <div className="text-[11px] leading-none">
            {saveState === 'error' ? (
              <span className="text-amber-400">{t('oeilAirbnbReport.autosave.failed')}</span>
            ) : saveState === 'saving' ? (
              <span className="text-[#AAA]">{t('oeilAirbnbReport.autosave.saving')}</span>
            ) : lastSavedAt ? (
              <span className="text-[#777]">{t('oeilAirbnbReport.autosave.savedAt', { time: lastSavedAt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) })}</span>
            ) : (
              <span className="text-[#777]">{t('oeilAirbnbReport.autosave.idle')}</span>
            )}
          </div>
