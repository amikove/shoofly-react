...
export default function NewMissionModal({ open, onClose, onCreated, preselectedOeil }) {
  ...
  const submit = async (e) => {
    ...
    try {
		...
		// Si un Œil est pré-sélectionné → l'assigner directement
		if (preselectedOeil?.id && data.mission?.id) {
		  try {
			await missionsAPI.accept(data.mission.id)
		  } catch {
			// L'accept peut échouer si seul l'Œil peut accepter — on ignore
		  }
		}
      ...
      <div className="bg-[#181818] border border-white/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-[0_24px_60px_rgba(0,0,0,0.6)]">
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-[#AAA] mt-0.5">Décrivez votre besoin</p>
          </div>
          ...
          {TYPES.map((t) => (
            <button key={t.id} onClick={() => setType(t.id)}
              className={`flex flex-col items-center gap-1 py-3 rounded-xl border text-xs font-medium transition-all ${
                type === t.id ? 'border-[#FF4D00] bg-[#FF4D00]/10 text-white' : 'border-white/12 bg-[#222] text-[#AAA] hover:border-white/22'
              }`}>
...
{preselectedOeil && (
  <div className="flex items-center gap-2 mb-4 p-3 bg-[#FF4D00]/10 border border-[#FF4D00]/25 rounded-xl">
    <span className="text-sm">👁️</span>
    <span className="text-sm font-medium">
      Mission assignée à <strong>{preselectedOeil.first_name} {preselectedOeil.last_name}</strong>
    </span>
  </div>
)}
        <form onSubmit={submit} className="space-y-3">
          <div>
            ...
            <input className="input" value={form.title} onChange={set('title')}
              placeholder={type === 'immobilier' ? 'Ex: Visite appartement Agdal' :
                          type === 'file_attente' ? 'Ex: File CNSS Hay Riad' :
                          type === 'audit' ? 'Ex: Audit café Hassan' : 'Décrivez en une phrase votre besoin'} required />
          </div>
          <div>
            <label className="label">Adresse complète *</label>
            <input className="input" value={form.address} onChange={set('address')} placeholder="Rue, quartier, ville" required />
          </div>
          <div>
            <label className="label">Description</label>
            <textarea className="input resize-none h-20" value={form.description} onChange={set('description')}
              placeholder={type === 'immobilier' ? 'Ex: Vérifier cuisine, salle de bain, pression eau...' : 'Instructions particulières...'} />
          </div>
          <div>
